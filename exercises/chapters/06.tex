...
\begin{table}[H]
    \begin{tabular}{|r@{\hskip -0.75pt} r|c c||c|c c||c|c c||c|c c||c|c c||c|}
        \hline
...
\begin{multicols}{2}
    ...
    \begin{figure}[H]
        \begin{tikzpicture}[shorten >=1pt,node distance=2Omm,on grid,auto, scale=0.9]
            \def\radius{18mm}
            ...
            (1) edge [loop above] node {$b$} ()
                edge [bend left=20] node[below] {$a$} (2)
