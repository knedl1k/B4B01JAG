...
\begin{tikzpicture}
    ...
    \node[state, right of=0] (5) {$5$};
    \node[state, right of=1] (1) {$1$};
    \node[state, below of=1] (3) {$3$};
    \node[state, below of=5] (2) {$2$};
    \node[state, below of=0] (7) {$7$};

    \draw
        (0) edge[loop above] node{$a$} (0)
        (0) edge[bend left, above] node{$b$} (5)
        (5) edge[loop above] node{$a$} (5)
        (5) edge[bend left, above] node{$b$} (1)
        (1) edge[loop above] node{$a$} (1)
        (1) edge[bend left, right] node{$b$} (3)
        (3) edge[loop below] node{$a$} (3)
        (3) edge[bend left] node[below]{$b$} (2)
        (2) edge[loop below] node{$a$} (2)
        (2) edge[bend left, below] node{$b$} (7)
        (7) edge[loop below] node{$a$} (7)
        (7) edge[bend left,left] node{$b$} (0)
        ;
\end{tikzpicture}
