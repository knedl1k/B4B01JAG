...
\begin{figure}[H]
    ...
    \begin{minipage}[c]{0.3\textwidth}
        \begin{tikzpicture}[shorten >=1pt, node distance=25mm, on grid, auto]
            ...
            (p) edge node {$b$} (q)
            (p) edge node {$c$} (r)
            (q) edge [loop above] node {$a$} ()
            (q) edge node {$b,c$} (r)
            (r) edge [loop below] node {$a, b, c$} ()
    ...
    \begin{minipage}[c]{0.7\textwidth}
        \begin{tabular}{|l|c c c|c|c c c|c|}
            ...
            $\phantom{\to}\bc{p, q}$   & $\bc{p, q}$   & $\bc{p, q, r}$ & $\bc{p, q, r}$ & $O$ & $ O $ & $ K $ & $ K $ & $O$ \\
